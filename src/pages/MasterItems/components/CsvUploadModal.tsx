...
export function CsvUploadModal({ isOpen, onClose, onUpload }: { isOpen: boolean, onClose: () => void, onUpload: (data: any[]) => void }) {
    ...
    return (
        <div className="fixed inset-0 z-[500] flex items-center justify-center bg-[#141619]/60 backdrop-blur-sm p-4 animate-fadeIn font-sans">
            <StandardModalWrapper className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl flex flex-col overflow-hidden max-h-[90vh] border border-white/40">
                <div className="p-5 border-b border-[#E6E1DB] flex justify-between items-center bg-[#2E395F] text-white modal-handle cursor-move shrink-0">
                    <h3 className="font-black flex items-center gap-2 uppercase tracking-widest text-sm pointer-events-none"><LucideIcon name="upload-cloud" /> Import Bulk CSV</h3>
                    ...
                    {!previewData.length ? (
                        <div className={`border-2 border-dashed rounded-xl p-10 text-center transition-all bg-[#F2F4F6]/50 ${dragActive ? 'border-[#C22D2E] bg-[#C22D2E]/5' : 'border-[#B2CADE]'}`} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
                            ...
                            <div className="mt-8 bg-[#F2F4F6] p-4 rounded-lg border border-[#E6E1DB] text-left">
                                ...
                                <p className="text-[11px] text-[#737597] mb-2">Please ensure your CSV file has the following headers exactly as shown:</p>
                        ...
                        <div className="overflow-x-auto border border-[#E6E1DB] rounded-lg max-h-[400px] custom-scrollbar shadow-inner text-[12px] flex flex-col">
                            <h4 className="font-bold text-[#2E395F] mb-3 px-4 pt-4 flex justify-between items-center shrink-0">
                            ...
                            <table className="w-full text-left whitespace-nowrap">
                                <thead className="bg-[#55738D] text-white sticky top-0 z-10">
                                    <tr>
                                        <th className="p-3 font-bold uppercase tracking-wider text-[10px]">SKU</th>
                                        ...
                                        <th className="p-3 font-bold uppercase tracking-wider text-[10px]">Brand</th>
                                        <th className="p-3 font-bold uppercase tracking-wider text-[10px] text-right">Weight</th>
                                        <th className="p-3 font-bold uppercase tracking-wider text-[10px] text-center">Status</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-[#E6E1DB]">
                                    {previewData.slice(0, 10).map((row, i) => (
                                        <tr key={i} className="hover:bg-[#F2F4F6]">
                                            <td className="p-3 font-mono font-bold text-[#C22D2E]">{row.SKU || '-'}</td>
                                            <td className="p-3 text-[#2E395F] font-bold">{row.Name || '-'}</td>
                                            <td className="p-3 text-[#55738D]">{row.Category || '-'}</td>
                                            <td className="p-3 text-[#737597]">{row.Brand || '-'}</td>
                                            <td className="p-3 text-[#2E395F] font-mono text-right">{row.Weight || '-'}</td>
                                            <td className="p-3 text-center">{row.Status || '-'}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                            {previewData.length > 10 && <div className="p-3 text-center text-[10px] text-[#737597] bg-[#F2F4F6] font-bold uppercase tracking-widest shrink-0 mt-auto">Showing first 10 rows of {previewData.length} records...</div>}
                        </div>
                    )}
                ...
                {previewData.length > 0 && (
                    <div className="p-4 border-t border-[#E6E1DB] flex justify-end gap-3 bg-[#F2F4F6] shrink-0">
