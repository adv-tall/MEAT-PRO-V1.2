...
export function CsvUploadModal({ isOpen, onClose, onUpload }: { isOpen: boolean, onClose: () => void, onUpload: (data: any[]) => void }) {
    ...
    const confirmUpload = () => {
        ...
        setPreviewData([]); 
        setError(null);
        Swal.fire({ icon: 'success', title: 'Imported!', timer: 1500, showConfirmButton: false });
    };
    ...
    return (
        <div className="fixed inset-0 z-[500] flex items-center justify-center bg-[#141619]/60 backdrop-blur-sm p-4 animate-fadeIn font-sans">
            <StandardModalWrapper className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] border border-white/40">
                ...
                <div className="p-8 flex-1 overflow-y-auto">
                    {!previewData.length ? (
                        <div className={`border-2 border-dashed rounded-xl p-10 text-center transition-all bg-[#F2F4F6]/50 ${dragActive ? 'border-[#C22D2E] bg-[#C22D2E]/5' : 'border-[#B2CADE]'}`} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
                            <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm text-[#55738D]"><LucideIcon name="file-up" size={32} /></div>
                        ...
                        <div className="overflow-x-auto border border-[#E6E1DB] rounded-lg max-h-[300px] custom-scrollbar shadow-inner text-[12px]">
                            <table className="w-full text-left whitespace-nowrap">
                                <thead className="bg-[#55738D] text-white sticky top-0">
                                    <tr><th className="p-3 font-bold uppercase tracking-wider text-[10px]">SFG_ID</th><th className="p-3 font-bold uppercase tracking-wider text-[10px]">FG_SKU</th><th className="p-3 font-bold uppercase tracking-wider text-[10px]">FG_Name</th></tr>
                                </thead>
                                <tbody className="divide-y divide-[#E6E1DB]">
                                    {previewData.slice(0, 10).map((row, i) => (
                                        <tr key={i} className="hover:bg-[#F2F4F6]">
                                            <td className="p-3 font-mono font-bold text-[#C22D2E]">{row.SFG_ID || '-'}</td>
                                            <td className="p-3 font-mono text-[#2E395F] font-bold">{row.FG_SKU || '-'}</td>
                                            <td className="p-3 text-[#55738D]">{row.FG_Name || '-'}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                    ...
                    {error && <div className="mt-4 p-3 bg-red-50 text-red-600 text-[12px] rounded-lg border border-red-100 flex items-center gap-2"><LucideIcon name="alert-circle" size={16}/> {error}</div>}
                </div>
                {previewData.length > 0 && (
                    <div className="p-4 border-t border-[#E6E1DB] flex justify-end gap-3 bg-[#F2F4F6] shrink-0">
                        ...
                        <button onClick={confirmUpload} className="px-8 py-2.5 bg-[#537E72] hover:bg-[#3d5e55] text-white font-black text-[12px] uppercase tracking-widest rounded-lg shadow-md transition-colors flex items-center gap-2">
