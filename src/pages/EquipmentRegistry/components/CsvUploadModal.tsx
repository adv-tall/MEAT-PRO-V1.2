...
export function CsvUploadModal({ isOpen, onClose, onUpload }: { isOpen: boolean, onClose: () => void, onUpload: (data: any[]) => void }) {
    ...
    const confirmUpload = () => {
        ...
        setPreviewData([]);
        setError(null);
        Swal.fire({ icon: 'success', title: 'Imported!', text: 'Equipment data has been successfully imported.', timer: 1500, showConfirmButton: false });
    };
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center bg-[#141619]/60 backdrop-blur-sm p-4 animate-fadeIn font-sans">
            <StandardModalWrapper className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh] border border-white/40">
                <div className="p-5 border-b border-[#E6E1DB] flex justify-between items-center bg-[#2E395F] text-white modal-handle cursor-move shrink-0">
                    <h3 className="font-black flex items-center gap-2 uppercase tracking-widest text-sm pointer-events-none"><LucideIcon name="upload-cloud" /> Import Equipment CSV</h3>
                            ...
                            <table className="w-full text-left whitespace-nowrap">
                                <thead className="bg-[#55738D] text-white sticky top-0 z-10">
                                    <tr>
                                        ...
                                        <th className="p-3 font-bold uppercase tracking-wider text-[10px]">Step</th>
                                        <th className="p-3 font-bold uppercase tracking-wider text-[10px] text-center">Quantity</th>
                                        <th className="p-3 font-bold uppercase tracking-wider text-[10px]">Note</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-[#E6E1DB]">
                                    {previewData.slice(0, 10).map((row, i) => (
                                        <tr key={i} className="hover:bg-[#F2F4F6]">
                                            ...
                                            <td className="p-3 text-[#55738D]">{row.Step || '-'}</td>
                                            <td className="p-3 text-[#2E395F] font-mono text-center">{row.Quantity || '-'}</td>
                                            <td className="p-3 text-[#737597]">{row.Note || '-'}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                            {previewData.length > 10 && <div className="p-3 text-center text-[10px] text-[#737597] bg-[#F2F4F6] font-bold uppercase tracking-widest">Showing first 10 rows of {previewData.length} records...</div>}
                        </div>
                    )}
