...
files.push(path.join(pagesDir, 'Home', 'index.tsx'));
...
files.forEach(file => {
    ...
    // Ensure padding pt-8 pb-10 px-8 on the page wrapper ONLY where we already have some padding, to normalize it
    content = content.replace(/className="([^"]+?)\spx-8 pb-10 pt-8"([^>]*>)/g, 'className="$1 px-8 pt-8 pb-10"$2');
    content = content.replace(/className="([^"]+?)\spt-8"([^>]*>)/g, 'className="$1 pt-8"$2');
